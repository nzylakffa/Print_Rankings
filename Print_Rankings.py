...
def generate_top_200_pdf(title, df):
    ...
    # ✅ Add Image (Top Right Corner)
    image_path = "ffa_red.png"  # Local path
    pdf.image(image_path, x=150, y=-2, w=20)  # Adjust X, Y, and width as needed
    pdf.image(image_path, x=100, y=-2, w=20)  # Adjust X, Y, and width as needed
...
with col2:
    ...
    for title, url in SHEET_URLS.items():
        try:
            ...
            top_200_pdf = generate_top_200_pdf(f"One Page Top 200 - {title}", df_top_200)
